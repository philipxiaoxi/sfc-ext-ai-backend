...
>
> 客户端在收到 `DONE "已停止"` 后，不应再期望收到与本次响应相关的任何消息。如因竞态收到少量残留消息（如被抑制的 `TOOL_CALL_END`），客户端可忽略 `DONE` 之后到达的残余消息。中断时未完成的流式回复不会保存到对话记忆（ChatMemory）；若中断前 LLM 已发起工具调用，该次调用请求会保留在记忆中，且服务端会为每个未完成的工具调用补写一条"工具调用已被用户中断"的占位结果，以保证后续对话的上下文完整性。
